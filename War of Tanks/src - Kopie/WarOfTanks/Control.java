...
public class Control extends Applet {
	...
	public int numberPlayers = 4, currentPlayerID, weaponPanelAnimation = 0;
	private double shotX, shotY, shotPower, shotDirection;

	private boolean drivingRight, drivingLeft;
	public boolean weaponPanelAnimationDirection, menuIsOpen = false;
	...
	public void init() {
		...
		Images[13] = getImage(codeBase, "weapon_panel_item_selected.png");
		Images[14] = getImage(codeBase, "weapon_panel_item.png");
		//---
		Images[12] = getImage(codeBase, "sign.png");
//		Images[13] = getImage(codeBase, "sign.png");
//		Images[14] = getImage(codeBase, "sign.png");
		//---
		Cursor cursor = Toolkit.getDefaultToolkit().createCustomCursor(getImage(codeBase, "menu_aim.png"), new Point(this.getX() + 15,  this.getY() + 16), "img");
		setCursor(cursor);
		...
		// TEST---------------
		Tanks[0] = new Tank("Dawidoooo", 250, 150, 0, 100, 100, 0, 250);
		Tanks[1] = new Tank("No0bSeb", 50, 200, 3, 150, 100, 0, 250);
		Tanks[2] = new Tank("DerKillerKing", 250, 150, 2, 300, 300, 0, 250);
		Tanks[3] = new Tank("Sebidooo", 125, 150, 1, 400, 100, 0, 250);
		currentPlayerID = 0;
		// --------------------
		try {
	...
	public void paint(Graphics g) {
		...
		int x = 170, y = 710 - weaponPanelAnimation;
		for (int i = 0; i <= Tanks[currentPlayerID].getNumberWeaponNames() - 1; i++) {
			if (i == Tanks[currentPlayerID].getSelectedWeapon()) {
				g.drawImage(Images[14], x, y, 170, 60, null);
				g.setColor(Color.white);
			} else {
				g.drawImage(Images[13], x, y, 170, 60, null);
				g.setColor(Color.black);
			}
			g.drawString(Tanks[currentPlayerID].getWeaponNames()[0][i], x + 5, y + 13);
			g.drawString(Tanks[currentPlayerID].getWeaponNames()[1][i], x + 150, y + 13);
			g.drawString(Tanks[currentPlayerID].getWeaponNames()[2][i], x + 5, y + 33);
			if (i == 4) {
				y += 73;
				x = -4;
			}
