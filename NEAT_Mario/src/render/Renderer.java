...
public class Renderer {
	...
	public static final void renderNetwork(Graphics2D g, Network network, int speciesNumber) {
		...
		if (RENDER_NETWORK) {
				...
				for (Gen gen : network.getGenome().getGenes()) {
					if (gen.isEnabled()) {
						RenderNeuron startRenderNeuron = (RenderNeuron) renderNeurons.stream()
								...
								.filter(renderNeuron -> renderNeuron.getNeuron().getId() == gen.getInputNeuronId()).findFirst()
								.orElse(null);

						RenderNeuron endRenderNeuron = (RenderNeuron) renderNeurons.stream()
								.map(renderObject -> (RenderNeuron) renderObject)
								.filter(renderNeuron -> renderNeuron.getNeuron().getId() == gen.getOutputNeuronId()).findFirst()
								.orElse(null);
			...
			renderNeurons.stream().forEach(neuron -> neuron.render(g));
			renderGens.stream().forEach(gen -> gen.render(g));
		}
